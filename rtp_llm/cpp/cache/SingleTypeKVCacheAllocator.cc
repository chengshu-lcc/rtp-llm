...
std::shared_ptr<KVCacheResource> SingleTypeKVCacheAllocator::incrKVCacheRef(const KVCacheResource& kvcache_resource,
    ...
    selected_resource->initGroups(1, config_.layer_all_num, config_.layer_to_group_id, config_.blocksPerKvBlock());
...
bool SingleTypeKVCacheAllocator::updateKVBlock(const BatchKVCacheResourcePtr& kv_cache_resource,
    ...
    // init for all batch
    kv_cache_resource->initGroups(1, config_.layer_all_num, config_.layer_to_group_id, config_.blocksPerKvBlock());
    ...
    for (int new_batch_idx = 0; new_batch_idx < new_batch_size; ++new_batch_idx) {
        ...
        if (fork_count == 1) {
            ...
            if (copy_last_block && !block_ids.blocks().empty()) {
                ...
                auto new_blocks = block_pool_->malloc(1);
                RTP_LLM_CHECK_WITH_INFO(!new_blocks.empty(),
                                        "malloc one block from pool failed during kv cache update");
                const int new_block = new_blocks[0];
