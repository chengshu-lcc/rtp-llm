...
public:
    ...
    // Null-out blocks that are no longer needed; uses BlockIds::remove() for the nullified IDs.
    ...
    // Append `new_block_indices` to `block_ids` via BlockIds::add() and reference them in the pool.
