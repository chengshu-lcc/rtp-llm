...
void StreamCacheResource::init(int batch_size) {
    ...
    if (resource_context_.cache_manager) {  // cache manager is null when warmup
        ...
        if (cache_config.kernel_seq_size_per_block > 0 && cache_config.seq_size_per_block > 0) {
            blocks_per_kv_block = cache_config.seq_size_per_block / cache_config.kernel_seq_size_per_block;
        }
    }

    batch_kv_cache_resource_->initGroups(group_nums, layer_all_num, layer_to_group, blocks_per_kv_block, group_types);
