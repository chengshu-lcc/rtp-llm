...
def split_qkvz_by_group(
    ...
    """
    # Calculate group_v: number of value heads per key head
    group_v = linear_config.linear_num_value_heads // linear_config.linear_num_key_heads

    # Size per head group: Q + K + V + Z
    group_size = (
    ...
    # Reshape to [hidden_size, head_num_k, group_size]
    t = t.view(hidden_size, total_groups, group_size)
    logging.info(
        f"[split_qkvz_by_group]: t.shape[hidden_size, head_num_k, group_size] = {t.shape}"
    )

    # Split by tp_rank along head_num_k dimension
    ...
    # Reshape back to [hidden_size, local_head_num_k * group_size]
...
def split_qkvz_by_group_t(
    ...
    """Split qkvz weight by group with transpose.

    Input layout: [head_num_k * group_size, hidden_size]
    Output layout: [local_head_num_k * group_size, hidden_size]
    """
...
def split_qkvz_channel_scale_by_group(
    ...
    # Reshape to [head_num_k, group_size, 1]
    ...
    # Reshape back to [local_head_num_k * group_size, 1]
