# -*- coding: utf-8 -*-
# Ported from sglang/python/sglang/srt/layers/attention/fla/fused_merge_recompute.py
#
# Fuses three GDN kernels (recompute_w_u_fwd_kernel + chunk_scaled_dot_kkt
# off-diagonal merge + diagonal-block invert) into a single launch. Consumes
# Ai16 = output of solve_tril_16x16_kernel; computes off-diagonal Ai blocks
# inline, then produces w (uses g_cumsum.exp()) and u (uses only beta).
...
def fused_merge_recompute(
    ...
    Args:
        ...
        beta: [B, T, H]
        g_cumsum: [B, T, H]
        A: [B, T, H, chunk_size]               (output of chunk_scaled_dot_kkt_fwd, fp32)
        Ai16: [B, T, H, 16]                    (diagonal block inverses from solve_tril_16x16, k.dtype)
